...
def onnx_to_tflite(onnx_file, tflite_file, tflite_file_int8, validation_dataset):
    # 1. add shape info (and save)
    onnx.save(onnx.shape_inference.infer_shapes(onnx.load(onnx_file)), onnx_file)
    ...
    # crashes on YOLOX model, disable for the time being
    if tflite_file_int8:
        try:
