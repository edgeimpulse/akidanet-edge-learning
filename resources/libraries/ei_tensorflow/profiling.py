...
def calculate_memory(model_file, model_type, mode, prepare_model_tflite_script, prepare_model_tflite_eon_script):
    ...
    # Some models don't have the scripts (e.g. akida) so skip this step
    elif prepare_model_tflite_script or prepare_model_tflite_eon_script:
        memory = {}
        ...
        if prepare_model_tflite_script:
            try:
                ...
                print('WARN: Failed to get memory (TFLite Micro): ', end='')
        ...
        if prepare_model_tflite_eon_script:
            try:
                ...
                print('WARN: Failed to get memory (EON): ', end='')
                print(err, flush=True)
                memory['eon'] = None
    else:
        memory = None
...
def get_model_metadata(keras_model, validation_dataset, Y_test, X_samples, Y_samples, has_samples,
    ...
    if file_akida:
        ...
        print("########################################")
        print(f"# Model sparsity: {sparsity:6.2f} %" + 13 * " " + "#")
        print(f"# Used NPs:   {total_nps:3d}" + 22 * " " + "#")
        print(f"# Used nodes: {nodes:3d}" + 22 * " " + "#")
        print("########################################")
        # TODO: is it correct for Akida?
        # akida_perf['estimatedMACCs'] = estimated_maccs
        metadata['availableModelTypes'].append(model_type)
...
def profile_tflite_file(file, mode, prepare_model_tflite_script, prepare_model_tflite_eon_script):
    ...
    try:
        ...
        result = subprocess.run(['/app/eon_compiler/compiler-cmsisnn', '--verify', file], stdout=subprocess.PIPE)
        if (result.returncode == 0):
            metadata['isSupportedOnMcu'] = True
        else:
            metadata['isSupportedOnMcu'] = False
            metadata['mcuSupportError'] = result.stdout.decode('utf-8')
        ...
        traceback.print_exc()
        metadata['isSupportedOnMcu'] = False
        metadata['mcuSupportError'] = str(err)
